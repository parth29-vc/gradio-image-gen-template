...
# List of LoRA repositories and their respective trigger words
lora_repos = [
    {"repo": "parth29vc/shm-gn", "trigger_word": "GN"},
    {"repo": "parth29vc/shm-bb", "trigger_word": "BB"},  # Add more LoRAs here
    {"repo": "parth29vc/shm-bm", "trigger_word": "BM"}  # Example of multiple LoRAs
]

# Load LoRAs sequentially
for lora in lora_repos:
    pipe.load_lora_weights(lora["repo"])
...
def run_lora(prompt, cfg_scale, steps, randomize_seed, seed, width, height, lora_scale, progress=gr.Progress(track_tqdm=True)):
    ...
    # Generate image using the pipeline, appending each LoRA's trigger word
    lora_prompt = prompt + " " + " ".join([lora["trigger_word"] for lora in lora_repos])
    image = pipe(
        prompt=lora_prompt,
...
with gr.Blocks() as app:
    gr.Markdown("# Flux SHMLORA Image Generator")
    with gr.Row():
        ...
        with gr.Column(scale=1):
            ...
            gr.Markdown("Generate images using Fux SHMLORA and a text prompt.\n[[non-commercial license, Flux.1 Dev](https://huggingface.co/black-forest-labs/FLUX.1-dev/blob/main/LICENSE.md)]")
